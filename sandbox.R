...
# answer : started to decrease from 1999 to 2002 and then started to to increase from 2002 to 2008
...
sum(baltimoreVehicleMotorIndex)
...
# answer : remained around 10

# ---------------------------------------------------------------------
# Compare emissions from motor vehicle sources in Baltimore City with emissions from motor vehicle sources  
# in Los Angeles County, California (fips == "06037"). 
laVehicleMotorIndex <- (NEI$SCC %in% vehicleMotorSccs & NEI$fips == "24510") 
sum(laVehicleMotorIndex)
laVehicleMotor <- NEI[laVehicleMotorIndex, ]

laVehicleMotorIndexPerYear <- aggregate(Emissions ~ year, laVehicleMotor, sum)
plot.new()
plot(laVehicleMotorIndexPerYear, type="l")

# Which city has seen greater changes over time in motor vehicle emissions?

# VERIFY WHY NO CHANGE AND SIMILAR VALUE?
